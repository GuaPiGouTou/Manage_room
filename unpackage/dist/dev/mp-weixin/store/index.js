...
const store = common_vendor.createStore({
  ...
  mutations: {
    ...
    UPDATE_BASE_INFO(state, payload) {
      ...
      common_vendor.index.setStorageSync("baseInfo", state.baseInfo);
    },
    // 初始化房间数据
    ...
    LOAD_FROM_STORAGE(state) {
      const baseInfo = common_vendor.index.getStorageSync("baseInfo");
      if (baseInfo) {
    ...
    CLEAR_ALL_DATA(state) {
      ...
      common_vendor.index.removeStorageSync("baseInfo");
      common_vendor.index.removeStorageSync("rooms");
    }
  },
  actions: {
    // 初始化房间
  ...
  getters: {
    ...
    isAllRoomsCompleted: (state) => {
      return state.rooms.every(
        (room) => room.roomNumber && room.roomType && room.videos.length > 0
      );
    }
  }
});
